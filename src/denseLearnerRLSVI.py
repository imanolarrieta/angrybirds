...
class RLSVI:
    ...
    def update_obs(self, ep, h, oldObs, reward, newObs):
        ...
        '''
        if ep >= self.maxHist:
            print('****** ERROR: HISTORY Memory Exceeded ******')
        ...
        if len(self.memory[h]['oldFeat']) == len(self.memory[h]['rewards']) \
           and len(self.memory[h]['rewards']) == len(self.memory[h]['newFeat']):
            ...
            print('****** ERROR: Memory LA dims Failure ******')
    ...
    def update_policy(self, ep):
        ...
        for i in range(H):
            ...
            mu = self.thetaMeans[h]
            sig = np.sqrt(self.covs[h].diagonal())
            if self.isRLSVI:
                self.thetaSamps[h] = (mu + sig*np.random.normal(size=self.nFeat)).T
            else:
                self.thetaSamps[h] = (mu).T # If epsilon>0.0, do not sample (and use epsilon-greedy exploration)
